import { getProjectTeam, removeProjectMember } from "@/api/TeamAPI";
...
export default function ProjectTeamView() {
    ...
    if (data) return (
        <>
            ...
            <p className="text-2xl font-light text-gray-500 mt-5">Administrar Equipo de trabajo del proyecto</p>
            <nav className="my-5 flex gap-3">
                <button type="button"
                    className="bg-purple-400 hover:bg-purple-500 px-10 py-3 text-white text-xl font-bold cursor-pointer transition-colors"
                    onClick={() => navigate(location.pathname + '?addMember=true')}>Agregar Colaborador</button>
                                ...
                                <Menu as="div" className="relative flex-none">
                                    ...
                                    >
                                        <MenuItems className="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white py-2 shadow-lg ring-1 ring-gray-900/5 focus:outline-none">
                                            <MenuItem>
                                                <button type='button' className='block px-3 py-1 text-sm leading-6 text-red-500' onClick={() => mutate({projectId, id: member._id})}>
